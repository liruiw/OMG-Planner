...
class Voxels(object):
    ...
    def __init__(
        self, data, dims, translate, scale, axis_order, as_dense=True, cb=True
    ):
        self.data = data
        if as_dense:
            self.voxel_num = dims[0] * dims[1] * dims[2]
            # self.sparse_data = np.zeros([3, self.voxel_num])
            # valid_data = dense_to_sparse(data)  # added
            # self.voxel_num = valid_data.shape[1]
            # self.sparse_data[:, :self.voxel_num] = valid_data
        self.dims = dims
        self.translate = translate
        if cb:
            # shift the origin, so that it moves to center
            voxel_delta = scale / dims[0]
            valid = np.array(np.where(data > 0))
            min_ = np.min(valid, axis=1).reshape(
                [
                    3,
                ]
            )
            translate = (
                np.array(translate).reshape(
                    [
                        3,
                    ]
                )
            ...
            self.translate = translate.tolist()
        self.scale = scale
