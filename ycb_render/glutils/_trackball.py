...
class Trackball(object):
    ...
    def drag_to(self, x, y, dx, dy):
        ...
        # self._set_orientation(*self._get_orientation())
    ...
    def _set_delta_orientation(self, theta, phi):
        """ Computes rotation corresponding to theta and phi. """

        angle = theta * (math.pi / 180.0)
        sine = math.sin(0.5 * angle)
        xrot = [1 * sine, 0, 0, math.cos(0.5 * angle)]
        angle = phi * (math.pi / 180.0)
        sine = math.sin(0.5 * angle)
        zrot = [0, 0, sine, math.cos(0.5 * angle)]
        delta_rotation = _q_add(xrot, zrot)
        self._rotation = _q_add(delta_rotation, self._rotation)
        self._model = _q_rotmatrix(self._rotation)
